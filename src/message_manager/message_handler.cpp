...
void MsgHandler::createSuggestion(dpp::cluster& bot, const dpp::message_create_t& event)
{
    ...
    { 
        dpp::embed result = dpp::embed()
            ...
            .set_author(user.format_username(), "", event.msg.author.get_avatar_url())
...
void MsgHandler::Btns::deleteSuggestionBtn(dpp::cluster& bot, const dpp::button_click_t& event)
{
    ...
    if (clicker == originalAuthor)
        ...
        event.reply(dpp::message("You aren't "+originalAuthor).set_flags(dpp::m_ephemeral));
}

void MsgHandler::Btns::editSuggestionBtn(dpp::cluster& bot, const dpp::button_click_t& event)
{
    std::string clicker = event.command.get_issuing_user().format_username();
    std::string originalAuthor = event.command.msg.embeds[0].author->name;
    if (clicker == originalAuthor)
...
void MsgHandler::ModalForms::showSuggestionEditModal(dpp::cluster& bot, const dpp::form_submit_t& event)
{
